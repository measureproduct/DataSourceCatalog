...
public class Test {
    ...
    public void testMeasure() {
        ...
        measure.getProperties().put("queryField", "return ArchimateView.latest.all.select(v|v.Represented.size>0).collect(v|v.Descriptor.collect(d|d.Content.split('\\\\W+').size).sum()+v.Represented.select(r|r.isKindOf(Element)).collect(c | c.Descriptor.collect(n|n.Content.split('\\\\W+').size).sum()).sum()).sum();");
