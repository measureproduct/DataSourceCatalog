...
public class Test {
    ...
    public void testMeasure() {
        ...
        measure.getProperties().put("queryField", "var lst=ArchimateView.latest.all.select(v|v.Represented.size>0); return lst.collect(v|v.Descriptor.collect(d|d.Content.split('\\\\W+').size).sum()+v.Represented.select(r|r.isKindOf(Element)).collect(c| c.Descriptor.collect(n|n.Content.split('\\\\W+').size).sum()).sum()).sum()/lst.size;");
